...
import { CheckCircle, XCircle, Shield, Thermometer, Droplets, Wind, Gauge, Leaf, BarChart2 } from 'lucide-react';
...
const LOGIN_SECTIONS    = ['Globe3D', 'PrimaryButton', 'PantallaCarga'];
...
const SUPERADMIN_SECTIONS = ['EmpresaRow', 'NuevaEmpresaModal', 'EditarEmpresaModal'];
...
const SAMPLE_EMPRESA_2: Empresa = {
  id: 2,
    ...
    <div style={{ display: 'flex', minHeight: '100vh', fontFamily: 'system-ui, -apple-system, sans-serif', background: '#f8fafc' }}>
      ...
      <aside style={{
        ...
        <nav style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          <NavGroup label="Super Admin"        ids={SUPERADMIN_SECTIONS} onNav={scrollTo} />
      ...
      <main style={{ flex: 1, padding: '40px 48px', boxSizing: 'border-box', maxWidth: 960 }}>
        ...
        {/* Globe3D */}
        <Section id="Globe3D" title="Globe3D" description="Globo terráqueo 3D animado con WebGL (cobe). Elemento decorativo del fondo de las pantallas de login y forgot password.">
          <div style={{ position: 'relative', width: '100%', height: 340, background: '#060b1a', borderRadius: 12, overflow: 'hidden' }}>
            <Globe3D />
          </div>
        </Section>
        ...
        {/* info de alcaldia*/}
        {/*IRSACard */}
        <Section id="IRSACard" title="IRSACard" description="Tarjeta del Índice de Riesgo Sanitario con valor numérico, barra de progreso y color según nivel. Escala 0–100.">
          <Row label="Riesgo crítico (83 / 100) — dato en vivo">
            <div style={{ width: 340 }}>
              <IRSACard irsa={83} irsaMax={100} descripcion="Riesgo crítico, intervención inmediata necesaria" esDatoReal />
            </div>
          </Row>
          <Row label="Riesgo óptimo (24 / 100)">
            <div style={{ width: 340 }}>
              <IRSACard irsa={24} irsaMax={100} descripcion="Nivel óptimo para operaciones normales" />
            </div>
          </Row>
          <Row label="Riesgo moderado (52 / 100)">
            <div style={{ width: 340 }}>
              <IRSACard irsa={52} irsaMax={100} descripcion="Riesgo moderado, monitoreo continuo recomendado" />
            </div>
          </Row>
        </Section>
        {/*VariableCard*/}
        <Section id="VariableCard" title="VariableCard" description="Tarjeta de variable ambiental con ícono, valor principal, sublabel y tag de riesgo opcional.">
          <Row label="Variables climáticas">
            <div style={{ width: 200 }}>
              <VariableCard
          ...
          <Row label="Contaminantes (backend: promediosPorContaminante)">
            <div style={{ width: 200 }}>
              <VariableCard
        ...
        {/* VariablesGrid */}
        <Section id="VariablesGrid" title="VariablesGrid" description="Grid de VariableCard con temperatura, humedad y calidad del aire de una alcaldía.">
        ...
        <Section id="SanidadChart" title="SanidadChart" description="Área chart de Recharts con índice de sanidad ambiental (línea azul) y tendencia (línea gris punteada).">
          <div style={{ height: 320, display: 'flex', flexDirection: 'column' }}>
            <SanidadChart data={DATA_CDMX} year={new Date().getFullYear()} alcaldiaName="CDMX" />
          </div>
        </Section>
        ...
        {/* Super Admin */}
        {/* EmpresaRow */}
        <Section id="EmpresaRow" title="EmpresaRow" description="Fila de tabla con logo, nombre, admin, estado de suscripción y botón de edición.">
          <div style={{ background: 'white', borderRadius: 12, border: '1px solid #f3f4f6', overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <tbody>
                <EmpresaRow empresa={SAMPLE_EMPRESA}   onEdit={() => {}} />
                <EmpresaRow empresa={SAMPLE_EMPRESA_2} onEdit={() => {}} />
              </tbody>
            </table>
          </div>
        </Section>
        ...
        {/* NuevaEmpresaModal */}
        <Section id="NuevaEmpresaModal" title="NuevaEmpresaModal" description="Modal para registrar una nueva empresa cliente con sus datos y los del administrador.">
          <Row label="Disparador">
            <button
              onClick={() => setShowNuevaEmpresa(true)}
              style={{ padding: '10px 22px', borderRadius: 10, border: 'none', background: '#6fa8f7', color: 'white', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}
            >
              Abrir NuevaEmpresaModal
            </button>
          </Row>
          {showNuevaEmpresa && (
            <NuevaEmpresaModal
              onClose={() => setShowNuevaEmpresa(false)}
              onSave={data => { console.log('Empresa creada:', data); setShowNuevaEmpresa(false); }}
            />
          )}
        </Section>

        {/* EditarEmpresaModal */}
        <Section id="EditarEmpresaModal" title="EditarEmpresaModal" description="Modal para editar los datos de una empresa cliente existente con campos precargados.">
          <Row label="Disparador">
            <button
              onClick={() => setShowEditarEmpresa(true)}
              style={{ padding: '10px 22px', borderRadius: 10, border: 'none', background: '#6fa8f7', color: 'white', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}
            >
              Abrir EditarEmpresaModal
            </button>
          </Row>
          {showEditarEmpresa && (
            <EditarEmpresaModal
              empresa={SAMPLE_EMPRESA}
              onClose={() => setShowEditarEmpresa(false)}
              onSave={data => { console.log('Empresa actualizada:', data); setShowEditarEmpresa(false); }}
            />
          )}
        </Section>
