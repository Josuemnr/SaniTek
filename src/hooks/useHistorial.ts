...
import { api, ALCALDIA_ID, type PuntoTendenciaApi } from '@/Services/backendApi';
import {
  DATA_CDMX,
  getMetricas,
  type DataPoint,
...
function puntosADataPoints(puntos: PuntoTendenciaApi[]): DataPoint[] {
  return puntos.map((p, i, arr) => {
    const indice = Math.round(p.promedioIrsa * 1000) / 10;
    const ventana = arr.slice(Math.max(0, i - 2), i + 1);
    const tendencia = Math.round(
      (ventana.reduce((s, w) => s + w.promedioIrsa, 0) / ventana.length) * 1000
    ) / 10;
    return { mes: etiquetaAMes(p.etiqueta), indice, tendencia };
  });
}

export function useHistorial(nombreAlcaldia: string | null, selectedYear: number) {
  const [allPuntos, setAllPuntos]       = useState<PuntoTendenciaApi[]>([]);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api.irsa
      .tendencia(id, 'MENSUAL', 48)
      .then((resp) => {
        setAllPuntos(resp.puntos);
        setAlcaldiaName(resp.nombreAlcaldia);

        const years = [
          ...new Set(resp.puntos.map((p) => etiquetaAnio(p.etiqueta)).filter(Boolean)),
        ].sort((a, b) => a - b);
  ...
  useEffect(() => {
    ...
    let filtered = allPuntos.filter((p) => etiquetaAnio(p.etiqueta) === selectedYear);

    if (selectedYear === currentYear) {
      filtered = filtered.filter((p) => etiquetaMesIndex(p.etiqueta) <= currentMonth);
    }
