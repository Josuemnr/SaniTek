...
export function getMetricas(data: DataPoint[]): Metricas {
  if (data.length === 0) {
    return {
      indiceActual: 0, variacionMes: 0, promedioMensual: 0,
      maximoRegistrado: 0, mesesAtrasMaximo: 0,
      tendencia: "Estable", tendenciaLabel: "Sin datos",
    };
  }

  const last     = data[data.length - 1].indice;
  const prev     = data.length >= 2 ? data[data.length - 2].indice : last;
  ...
  return {
    ...
    tendenciaLabel: variacion > 0 ? "Mejorando" : variacion < 0 ? "Descendiendo" : "Estable",
  };
}
